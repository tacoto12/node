...
function keyCheck(val1, val2, strict, memos, iterationType, aKeys) {
  ...
  for (const key of aKeys) {
    if (!hasOwnProperty(val2, key)) {
      return false;
    }
  }

  if (strict && arguments.length === 5) {
    const onlySymbols = ONLY_ENUMERABLE & SKIP_STRINGS;
    const symbolKeysA = getOwnNonIndexProperties(val1, onlySymbols);
    const symbolKeysB = getOwnNonIndexProperties(val2, onlySymbols);
