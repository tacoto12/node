...
function areSimilarTypedArrays(a, b, isFloat, strict) {
  const len = a.byteLength;
  if (len !== b.byteLength) {
    return false;
  }
  const manualComparison = isFloat ? (strict ? false : true) : (len < 300);
  if (manualComparison) {
...
function innerDeepEqual(val1, val2, strict, memos) {
  ...
  if (val1Tag === '[object Array]') {
    ...
    return keyCheck(val1, val2, strict, memos);
  }
  ...
  if (isDate(val1)) {
    ...
    if (!areSimilarTypedArrays(val1,
                               val2, isFloatTypedArrayTag(val1Tag), strict)) {
      return false;
    }
    ...
    return keyCheck(val1, val2, strict, memos, val1.length,
                    val2.length);
    ...
    if (val1Value !== val1) {
      if (!innerDeepEqual(val1Value, val2.valueOf(), strict))
        return false;
      ...
      return keyCheck(val1, val2, strict, memos, lengthval1,
                      lengthval2);
    }
  }
  return keyCheck(val1, val2, strict, memos);
