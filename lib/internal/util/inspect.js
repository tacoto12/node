...
function getConstructorName(obj, ctx, recurseTimes) {
  ...
  const newCtx = { ...ctx, customInspect: false };
  return `<${formatValue(newCtx, firstProto, recurseTimes)}>`;
}
...
function formatValue(ctx, value, recurseTimes, typedArray) {
  ...
  if (ctx.stop !== undefined) {
    const name = getConstructorName(value, ctx, recurseTimes) ||
  ...
  if (ctx.customInspect) {
    ...
    if (typeof maybeCustom === 'function' &&
      ...
      const ret = maybeCustom.call(value, recurseTimes, plainCtx);
