...
function strictDeepEqual(val1, val2, memos) {
  ...
  if (isDate(val1)) {
    ...
    let getPrimitive = symbolValueOf;
    if (isNumberObject(val1)) {
      getPrimitive = numberValueOf;
    } else if (isStringObject(val1)) {
      getPrimitive = stringValueOf;
    } else if (isBooleanObject(val1)) {
      getPrimitive = booleanValueOf;
    } else if (isBigIntObject(val1)) {
      getPrimitive = bigIntValueOf;
    }
    try {
      if (!innerDeepEqual(getPrimitive(val1), getPrimitive(val2), kStrict)) {
        return false;
      }
    } catch {
      return false;
...
function looseDeepEqual(val1, val2, memos) {
  ...
  if (val1Tag === val2Tag) {
    ...
    if (isNativeError(val1) && isNativeError(val2)) {
